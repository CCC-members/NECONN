classdef frequency_bands_guide < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function AgreeButtonPushed(app, event)
            ...
            change_xml_parameter(file_path,root_tab,[parameter_name],[parameter_value],cell(0,0));
            
            parameters_name = ["delta_band","theta_band","alpha_band","beta_band"];
            parameters_value = ["null","null","null","null"];
            attributes =  cell(1,4);
            attributes{1,1} = ["end","4","run","false","start","0.1","name","delta"];
            attributes{1,2} = ["end","7","run","false","start","4","name","theta"];
            attributes{1,3} = ["end","14","run","false","start","7","name","alpha"];
            attributes{1,4} = ["end","31","run","false","start","14","name","beta"];
            
            
            
            if(app.CheckBox_Delta.Value)
                if(app.Spinner_delta_start.Value <= app.Spinner_delta_end.Value)
                    attributes{1,1} = ["end",app.Spinner_delta_end.Value,"run","true",...
                        "start",app.Spinner_delta_start.Value,"name","delta"];
                else
            ...
            if(app.CheckBox_theta.Value)
                if(app.Spinner_theta_start.Value <= app.Spinner_theta_end.Value)
                    attributes{1,2} = ["end",app.Spinner_theta_end.Value,"run","true",...
                        "start",app.Spinner_theta_start.Value,"name","theta"];
                else
            ...
            if(app.CheckBox_alpha.Value)
                if(app.Spinner_alpha_start.Value <= app.Spinner_alpha_end.Value)
                    attributes{1,3} = ["end",app.Spinner_alpha_end.Value,"run","true",...
                        "start",app.Spinner_alpha_start.Value,"name","alpha"];
                else
            ...
            if(app.CheckBox_beta.Value)
                if(app.Spinner_beta_start.Value <= app.Spinner_beta_end.Value)
                    attributes{1,4} = ["end",app.Spinner_beta_end.Value,"run","true",...
                        "start",app.Spinner_beta_start.Value,"name","beta"];
                else
            ...
            if(allOk)
                ...
                change_xml_parameter(file_path,root_tab,parameters_name,parameters_value,attributes);
